...
Name: "{autoprograms}\VoiceTyping (Режим диагностики)"; Filename: "{app}\VoiceTyping_Debug.bat"; WorkingDir: "{app}"; IconFilename: "{app}\app.ico"
...
begin
  try
    WizardForm.PageNameLabel.Font.Color := $001B7AFF; // Vibrant Orange (#FF7A1B in BGR)
    WizardForm.PageNameLabel.Font.Style := [fsBold];

    AccentLine := TPanel.Create(WizardForm);
    AccentLine.Parent := WizardForm.MainPanel;
    AccentLine.Left := 0;
    AccentLine.Top := WizardForm.MainPanel.Height - ScaleY(2);
    AccentLine.Width := WizardForm.MainPanel.Width;
    AccentLine.Height := ScaleY(2);
    AccentLine.BevelOuter := bvNone;
    AccentLine.Color := $001B7AFF;
  except
  end;
end;
...
begin
  Result := True;
  Form := CreateCustomForm(ScaleX(460), ScaleY(340), False, False);
  try
