...
def play_start_sound(volume: float = 0.28):
    ...
    if volume <= 0.01:
        return
    tone1 = _generate_bell_tone(523.25, 0.075, volume * 0.75)
    silence = np.zeros(int(_SAMPLE_RATE * 0.01), dtype=np.float32)
    tone2 = _generate_bell_tone(659.25, 0.095, volume)
...
def play_stop_sound(volume: float = 0.24):
    ...
    if volume <= 0.01:
        return
    tone1 = _generate_bell_tone(659.25, 0.060, volume * 0.70)
    silence = np.zeros(int(_SAMPLE_RATE * 0.01), dtype=np.float32)
    tone2 = _generate_bell_tone(523.25, 0.090, volume * 0.85)
    chime = np.concatenate([tone1, silence, tone2])
    _play_async(chime)
