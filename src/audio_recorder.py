    ...
    def start(self, chunk_callback=None, level_callback=None, device=None):
        ...
        with self._lock:
            ...
            try:
                ...
                print(f"[WARN] Ошибка старта аудиопотока с устройством {dev_to_use}: {e}")
                if dev_to_use is not None:
                    try:
                        print("[INFO] Попытка переключения на микрофон по умолчанию...")
                        self._stream = sd.InputStream(
                            samplerate=self.sample_rate,
                            channels=self.channels,
                            dtype="int16",
                        ...
                        print(f"[ERROR] Ошибка старта аудиопотока по умолчанию: {e2}")
                        self.is_recording = False
                        self._stream = None
                else:
                    self.is_recording = False
