...
# Russian conjunctions and words that almost always require a preceding comma
COMMA_WORDS = {
...
# Voice punctuation commands in Russian
VOICE_PUNCTUATION = [
...
def format_live_text(raw_text: str) -> str:
    ...
    # 1. Process explicit voice punctuation commands (e.g., "привет запятая как дела" -> "привет, как дела")
    ...
    # 2. Heuristic Russian auto-punctuation before subordinate clauses
    words = text.split()
    if not words:
        return ""
    ...
    for i, w in enumerate(words):
        ...
        # If current word requires a comma before it and previous word doesn't already have one
    ...
    # 3. Capitalization of first letter and sentences
